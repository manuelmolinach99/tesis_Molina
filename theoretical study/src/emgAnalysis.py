...
file = open('C:/Users/Ing. Biologica/Documents/tesis_molina/theoretical study/src/MyoArmband_data.pickle', 'rb')
...
C1 = 1 - 1 # Sensor
C2 = 5 - 1 # Sensor
EMG[:, C1] = (EMG[:,C1]- np.mean(EMG[:, C1]))/np.std(EMG[:, C1]) 
EMG[:, C2] = (EMG[:,C2]-np.mean(EMG[:, C2]))/np.std(EMG[:, C2])
...
ax[0].plot(t, EMG[:, C1], label = "Etiquetas sin corregir")
ax[0].vlines(t[eventos_1], ymin_1, ymax_1, color = 'black')
ax[1].plot(t, EMG[:, C1], label = "Etiquetas corregidas")
